...
모바일 접근성은 screen reader, touch target, focus flow, contrast, dynamic text, reduce motion, keyboard, safe area를 기준으로 검토한다. 핵심 사용자 여정에서 접근성 blocker가 있으면 release readiness도 낮춘다.
...
- loading, offline, permission denied, form error 상태는 screen reader가 식별할 수 있어야 한다.

## touch target

- 주요 tap target은 충분한 touch target size와 간격을 가진다.
...
- keyboard 입력 화면은 keyboard avoidance, submit, next, dismiss 흐름이 자연스러워야 한다.
- back gesture와 hardware back button 동작을 명확히 정의한다.
- safe area, notch, home indicator, Android navigation bar 때문에 주요 control이 가려지지 않아야 한다.
...
- reduced motion 설정에서 필수 정보가 animation에만 의존하지 않아야 한다.
...
- iOS와 Android 중 최소 한 플랫폼에서 screen reader label, dynamic type/font scaling, reduced motion, touch target size, contrast, keyboard avoidance를 실제 설정으로 점검하고 evidence를 남긴다.
