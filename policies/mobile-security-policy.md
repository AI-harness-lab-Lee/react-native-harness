...
- `async-storage`, plain file, sqlite plain table에는 token을 저장하지 않는다. token이 `async-storage`에 저장되면 차단 이슈다.
...
- PII가 암호화, 보관 기간, 삭제 정책 없이 local storage에 저장되면 차단 이슈다.
...
- production build에 dev endpoint 또는 secret이 포함되면 차단 이슈다.
